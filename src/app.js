...
function App() {
        ...
        <div id="timeline" className="w-full max-w-7xl px-4 sm:px-6 mb-16">
          <div className="bg-[#111827] p-8 md:p-12 rounded-[2rem] shadow-xl border border-white/10">
            ...
            <div className="relative flex justify-between items-start mb-10">
              <div className="absolute w-full h-1 bg-[#0A1F44] left-0" style={{ top: '48px' }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-fade-in-up">
            <div className="md:col-span-1 flex justify-center items-stretch">
                <div className="bg-[#0A1F44] p-6 rounded-2xl shadow-xl border border-[#2979FF]/20 flex flex-col items-center text-center transition-all duration-300 hover:scale-[1.03] hover:shadow-2xl hover:border-[#FFD700]">
                    ...
                    <p className="text-[#94A3B8] text-sm mt-1">{getLeadershipRegion(leadership.majorityLeader)}</p>
                </div>
            </div>
            <div className="md:col-span-1 flex justify-center items-stretch">
                <div className="bg-[#0A1F44] p-8 rounded-2xl shadow-xl border border-[#2979FF]/20 flex flex-col items-center text-center transition-all duration-300 hover:scale-[1.03] hover:shadow-2xl hover:border-[#FFD700]">
                    ...
                    <p className="text-[#94A3B8] text-sm mt-1">{getLeadershipRegion(leadership.speaker)}</p>
                </div>
            </div>
            <div className="md:col-span-1 flex justify-center items-stretch">
                <div className="bg-[#0A1F44] p-6 rounded-2xl shadow-xl border border-[#2979FF]/20 flex flex-col items-center text-center transition-all duration-300 hover:scale-[1.03] hover:shadow-2xl hover:border-[#FFD700]">
                    ...
                    <p className="text-[#94A3B8] text-sm mt-1">{getLeadershipRegion(leadership.minorityLeader)}</p>
                </div>
            </div>
          </div>
  ...
  const LegislativeActivity = () => {
    ...
    // Filter bills based on search term - now searches ALL bills when searching
    const filteredBills = bills.filter(bill => {
      // First check if bill matches search term
      ...
      // If there's a search term, show ALL matching bills regardless of status
      if (searchTerm.trim()) {
        return matchesSearch;
      }
      
      // If no search term, only show active bills (excluding completed/finalized bills)
      const isActiveBill = !['enacted', 'failed', 'vetoed', 'override won', 'override lost'].includes(bill.status);
      return isActiveBill;
    });
        ...
        <div className="bg-[#111827] p-8 rounded-2xl shadow-xl border border-white/10 mb-8">
          ...
          <div className="space-y-4">
            {agenda && agenda.length ? (
              ...
              <div className="text-center text-[#94A3B8] p-8">No upcoming work.</div>
            )}
          </div>
              ...
              <div className="mt-3 text-center">
                <p className="text-[#94A3B8] text-sm">
                  {filteredBills.length > 0 
                    ? `Found ${filteredBills.length} bill(s) matching "${searchTerm}"`
                    : `No bills found matching "${searchTerm}"`
                  }
                </p>
                <p className="text-[#64748B] text-xs mt-1">
                  Search includes all bills in the database, including completed ones
                </p>
              </div>
            )}
          </div>
...
// update raw vote text while typing (preserve commas/spaces)
const handleVoteTextChange = (e, key) => {
  const value = e.target.value;
  setBillForm(prev => ({ ...prev, voteText: { ...prev.voteText, [key]: value } }));
};

const handleVoteChange = (e, type) => {
  // kept for compatibility if you still call it, but admin UI will use handleVoteTextChange
  const { value } = e.target;
  const votesArray = value.split(',').map(name => name.trim()).filter(name => name);
  setBillForm(prevState => ({ ...prevState, vote: { ...prevState.vote, [type]: votesArray }, voteText: { ...prevState.voteText, [type]: value } }));
};

    const handleSubmitBill = async (e) => {
      e.preventDefault();
      // parse voteText into vote arrays
      const parsedVote = {
      ...
      setBillForm({ id: '', title: '', status: '', type: '', date: '', latestAction: '', sponsor: '', vote: { ayes: [], nays: [], abstain: [], absent: [] }, voteText: { ayes: '', nays: '', abstain: '', absent: '' } });
...
if (isEditingRep) {
        ...
        <div className="w-full max-w-7xl mx-auto space-y-8">
              ...
              <div className="md:col-span-2">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <input
                    ...
                    value={billForm.voteText?.ayes || ''}
                    onChange={(e) => handleVoteTextChange(e, 'ayes')}
                    className="w-full px-4 py-3 bg-[#111827] text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-[#2979FF] transition-all duration-300 shadow-inner placeholder-[#64748B]"
                  />
                  <input
                    type="text"
                    placeholder="Nays (comma-separated)"
                    value={billForm.voteText?.nays || ''}
                    onChange={(e) => handleVoteTextChange(e, 'nays')}
                    className="w-full px-4 py-3 bg-[#111827] text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-[#2979FF] transition-all duration-300 shadow-inner placeholder-[#64748B]"
                  />
                  <input
                    type="text"
                    placeholder="Abstain (comma-separated)"
                    value={billForm.voteText?.abstain || ''}
                    onChange={(e) => handleVoteTextChange(e, 'abstain')}
                    className="w-full px-4 py-3 bg-[#111827] text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-[#2979FF] transition-all duration-300 shadow-inner placeholder-[#64748B]"
                  />
                  <input
                    type="text"
                    placeholder="Absent (comma-separated)"
                    value={billForm.voteText?.absent || ''}
                    onChange={(e) => handleVoteTextChange(e, 'absent')}
                    className="w-full px-4 py-3 bg-[#111827] text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-[#2979FF] transition-all duration-300 shadow-inner placeholder-[#64748B]"
                  />
                </div>
              </div>
          ...
          <div className="bg-[#0A1F44] p-8 rounded-[2rem] shadow-xl border border-[#2979FF]/30 backdrop-blur-md animate-fade-in-up">
            ...
            <div className="mt-8 bg-[#111827] p-6 rounded-xl border border-[#2979FF]/20">
              <h3 className="text-xl font-bold text-[#FFD700] mb-4">All Representatives</h3>
              <div className="overflow-x-auto">
                <table className="min-w-full text-white table-auto">
                  <thead>
            ...
            <div className="mt-8 bg-[#0A1F44] p-6 rounded-xl border border-[#2979FF]/20">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xl font-bold text-[#FFD700]">Manage Duma Agenda</h3>
                      ...
                      {agendaForm.business.map((item, i) => (
                        <div key={i} className="bg-[#0A1F44] p-3 rounded space-y-3">
                          <div className="grid grid-cols-1 md:grid-cols-4 gap-2 items-center">
                            ...
                            <div className="flex items-center space-x-2">
                              <button type="button" onClick={() => removeAgendaItemField(i)} className="px-2 py-1 bg-red-500 rounded text-white">Remove</button>
                            </div>
                          </div>
                            ...
                            {item.items && item.items.map((bill, billIndex) => (
                              <div key={billIndex} className="bg-[#111827] p-2 rounded space-y-2">
                                <div className="flex justify-between items-center">
                                  <span className="text-xs text-[#94A3B8]">Bill {billIndex + 1}</span>
                                  <button
                                    type="button"
                                    onClick={() => {
  ...
  // Footer component
  const Footer = () => (
    <footer className="bg-[#0A1F44] text-[#94A3B8] py-8 border-t border-[#2979FF]/30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <p>&copy; 2025 State Duma of the Russian Federation. All rights reserved.</p>
      </div>
    </footer>
  );
